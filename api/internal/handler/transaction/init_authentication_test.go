...
import (
	...
	"net/http"
	"net/http/httptest"
	"pg/api/internal/mocks/service/transaction"
...
func TestInitAuthentication(t *testing.T) {
	type initAuthentication struct {
		mockInCard   model2.Card
		mockInOrder  model2.Order
		mockOutCard  model2.Card
		mockOutOrder model2.Order
		mockOutTrans model2.Transaction
		mockErr      error
		mockErrB     error
	}
	type arg struct {
